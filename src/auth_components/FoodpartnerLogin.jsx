...
//         console.log(form)
//         e.preventDefault();
//         const response=await axios.post("http://localhost:3000/api/auth/food-partner/login",form,{
...
const FoodpartnerLogin = () => {
  ...
  const handleSubmit = async (e) => 
    ...
    console.log(response.data);
    navigate(`/self/food-partner/${response.data.fpid._id}`); 
  }
  catch(error)
  {
    if (error.response && error.response.data.message) {
      alert(error.response.data.message); // show "User not found" or "Invalid username or password"
    } else {
